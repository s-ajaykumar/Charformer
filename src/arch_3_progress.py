...
class CharAttention(nn.Module):
    def __init__(self):
        ...
        self.register_buffer('tril', torch.tril(torch.ones((config.c_block_size, config.c_block_size), dtype = torch.float, device = config.device)))
    ...
    def forward(self, x, attention_mask):
        ...
        v = self.value(x)                               # B, W, c, C
        att_sc = 1.0 / attention_mask.sum(-1).float() 
        att_sc = att_sc.unsqueeze(-1).unsqueeze(-1)  # B, W, c, c
        out = v * att_sc  
        out = out * attention_mask.unsqueeze(-1)  # B, W, c, C      
...
class GPT(nn.Module):
    def __init__(self):
        ...
        self.ln5 = nn.LayerNorm(config.c_block_size*config.n_embd)  # Layer norm for the final output
    ...
    def forward(self, x, attention_mask, targets = None):
        ...
        x = x.view(B, W, c*config.n_embd)  # B, W, c, C
        x = self.ln5(x)
        x = x.view(B, W, config.c_block_size, config.n_embd)
        x = x * attention_mask.unsqueeze(-1)  # B, W, c_block_size, 1
        x = x.sum(dim = -2)  # B, W, C     
